...
for (let i = 0; i < graduates.length; i++) {
  firstName.push(graduates[i].first_name); // add firstnames to new firstname array
  emailAddress.push(graduates[i].email); // add emails to other new email array
}

for (let i = 0; i < firstName.length; i++) {
  for (let i = 0; i < emailAddress.length; i++) {
    contactInfo.push(firstName[i] + " " + emailAddress[i]);
  }
}
...
zooAnimals.forEach(currentValue => {
  animalNames.push({Name: currentValue.animal_name,
...
/* or could change the order:

const largerPopulation = [];

largerPopulation.push(
  zooAnimals.filter(currentValue => {
...
populationTotal.push(
  zooAnimals.reduce ((accumulator, currentValue) => {
